...
for(const el of lessons){
    if(el.name.includes('1x')){
        minAdam += parseInt(el.length.split(':')[0]);
        secAdam += parseInt(el.length.split(':')[1]);
    } else if (el.name.includes('2x')){
        minPrzemek += parseInt(el.length.split(':')[0]);
        secPrzemek += parseInt(el.length.split(':')[1]);
    } else if (el.name.includes('3x')){
        minMarcin += parseInt(el.length.split(':')[0]);
        secMarcin += parseInt(el.length.split(':')[1]);
    }
}

const timeAdam = new Date(0, 0, 0, 0, `${minAdam}`, `${secAdam}`);
const timePrzemek = new Date(0, 0, 0, 0, `${minPrzemek}`, `${secPrzemek}`);
const timeMarcin = new Date(0, 0, 0, 0, `${minMarcin}`, `${secMarcin}`);

const timeInformation = `Moduł Adama trwa ${timeAdam.getHours()} godziny, ${timeAdam.getMinutes()} minut i ${timeAdam.getSeconds()} sekund. Moduł Przemka trwa ${timePrzemek.getHours()} godziny, ${timePrzemek.getMinutes()} minut i ${timePrzemek.getSeconds()} sekund. Moduł Marcina trwa ${timeMarcin.getHours()} godziny, ${timeMarcin.getMinutes()} minut i ${timeMarcin.getSeconds()} sekund.`;
